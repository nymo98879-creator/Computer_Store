...
@section('title', 'Category 3 Products')
...
<div class="p-8 bg-gray-100 min-h-screen" x-data="{
    ...
    <div class="bg-white rounded-2xl shadow overflow-hidden">
        <table class="min-w-full border-collapse">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    {{-- <td class="py-3 px-4 text-center">
                        <div class="flex justify-center space-x-2">
                            ...
                            <form action="{{ route('admin.dproduct.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6" class="py-4 px-4 text-center text-gray-500">No products in this category.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="showEditForm" x-transition.opacity class="fixed inset-0 bg-gray-500/50 flex items-center justify-center z-50" x-cloak>
        <div @click.away="showEditForm = false" x-transition.scale
            ...
            <form :action="`/admin/dcategory-accessories/${editProduct.id}`" method="POST" enctype="multipart/form-data" class="space-y-3">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" x-model="editProduct.name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" required>
                </div>

                <!-- Description -->
                <div>
                    <label class="block text-gray-600 mb-1">Description</label>
                    <textarea name="description" rows="3" x-model="editProduct.description" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500"></textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" name="price" x-model="editProduct.price" step="0.01" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div>
                        <label class="block text-gray-600 mb-1">Stock</label>
                        <input type="number" name="stock" x-model="editProduct.stock" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" required>
                    </div>
                </div>
                    ...
                    <div class="flex gap-2 flex-wrap">
                        <template x-for="(img,index) in editProduct.images" :key="img.id">
                            <div class="relative">
                                ...
                                <button type="button" @click="editProduct.images.splice(index,1)" class="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">×</button>
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <button type="button" @click="showEditForm=false" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg">Cancel</button>
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Update Product</button>
                </div>
            </form>
        </div>
    </div>
